...
import { highDfMinimum, pruneTerms, MIN_MATCH_TERMS } from '../../src/features/search/droplist.js';
...
describe('the droplist is derived from the library, not shipped with the code', () => {
  sqliteIt('prunes a term this corpus is saturated with, which no English stoplist holds', async () => {
    ...
    // matches every item in the library, because `zoteus` does.
    expect((await keys(index, 'zoteus')).length).toBe(items.length);
    await index.close();
  });
  ...
  sqliteIt('leaves an index built before this change filtering nothing', async () => {
    ...
    expect((await keys(reopened, 'zoteus tomatoes greenhouse', 20)).length).toBe(items.length);
...
describe('the pruning rule itself', () => {
  ...
  it('returns the caller set untouched when too few terms survive', () => {
    const terms = ['to', 'be', 'or', 'not'];
    expect(pruneTerms(terms, (t) => t !== 'not', MIN_MATCH_TERMS)).toEqual(terms);
    expect(pruneTerms(terms, (t) => t === 'to' || t === 'be', MIN_MATCH_TERMS)).toEqual(['or', 'not']);
    // No droplist at all is not the same as an empty one, and neither prunes.
    expect(pruneTerms(terms, undefined, MIN_MATCH_TERMS)).toEqual(terms);
  });
});
