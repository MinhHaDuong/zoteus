...
Two consequences worth knowing. A query in which **fewer than two terms survive** is sent
unpruned — `to be or not to be` keeps only `not` otherwise, which retrieves nothing to do
with Hamlet — so a query made entirely of common words still answers on all of them rather
than on one incidental survivor. And an index built by an earlier version prunes nothing
