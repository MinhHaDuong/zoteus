...
| The embedding model changed | Only the changed items would come back with vectors in the new space; the rest would be ranked against a foreign one. (Same rule as [Changing the model](#choosing-and-tuning-embeddings).) |
...
Unset local configuration preserves the existing `minilm-l6-v2` chain. The sole curated
alternative is the measured multilingual `multilingual-e5-small-q8` chain:
...
prefixes, tokenizer window, and vector width. The execution provider is pinned to CPU and
must pass the local compatibility probe before the index is opened. Arbitrary Hugging Face
names and per-field overrides are deliberately rejected: a combination that merely loads
can still produce incompatible or incorrectly interpreted vectors.
