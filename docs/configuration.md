...
| `ZOTEUS_INDEX_AUTO_REFRESH` | `true` | Keep the search index current automatically: before answering a `zotero_semantic_search`, ask Zotero for the library's `Last-Modified-Version` and, when it has moved, index what changed and drop what was deleted before serving. Costs **one request** per query while the library has not changed, and nothing at all when Zotero is unreachable — the index is then served as it stands rather than the search failing. Requires `ZOTEUS_SEARCH_BACKEND=sqlite`; the resident JSON index has no cheap partial write, so it ignores this and keeps its rebuild-only behaviour. See [`semantic-search.md`](./semantic-search.md#keeping-the-index-current). |
| `ZOTEUS_SEARCH_BACKEND` | `json` | Where the keyword side of the index lives. `json` is the resident BM25 index, persisted to `search-index.json`. `sqlite` keeps passages in an on-disk FTS5 table (`search-index.sqlite`, beside the JSON file) and never loads them into the heap — the option for libraries large enough to need `--max-old-space-size`. Requires Node ≥ 22.5 for `node:sqlite`. On this backend there is no JSON snapshot: nothing is written back, because the database is the index. The first startup after switching migrates an existing `search-index.json` into the database — streamed, so it works on index files too large for `JSON.parse` — and leaves the JSON file untouched, so setting this back to `json` restores the previous index unchanged. Embedding vectors are not carried over by the migration: the log says how many were dropped, and the next index build regenerates them. New vectors are stored in the database too, in a `vec0` table, which needs the optional `sqlite-vec` package (`npm i sqlite-vec`); without it the SQLite backend runs keyword-only and says so under `vectorReason`. |
