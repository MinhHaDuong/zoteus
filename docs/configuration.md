...
| `ZOTEUS_EMBED_BATCH_SIZE` | `32` | Passages per embedding call: one API request, or one local pipeline call. Lower it when a provider rejects a batch outright (OpenAI answers `400` above 300K tokens per request, a ceiling full-text passages reach far sooner than metadata ones). Under Electron (Claude Desktop) the **local** embedder is capped at `8` whatever this says, because a pipeline call large enough to need a gigabyte in one block is refused by Chromium's allocator and kills the process outright ([#37](https://github.com/oscardvs/zoteus/issues/37)); a value below the cap is still honoured, and the server logs one line when the cap bites. See [`semantic-search.md`](./semantic-search.md#full-text-builds-inside-claude-desktop). |
