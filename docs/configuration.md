...
| `ZOTEUS_EMBEDDING_MODEL` | provider default | Embedding model for whichever provider `ZOTEUS_EMBEDDINGS` selected, **`local` included**: `Xenova/all-MiniLM-L6-v2` for `local`, `text-embedding-3-small` for `openai`, `text-embedding-004` for `gemini`. Under `local` it names any transformers.js feature-extraction model, which is how a non-English library gets an embedder trained for it (`Xenova/multilingual-e5-small` is the multilingual pick, same 384 dimensions, ~118 MB downloaded once into `<ZOTEUS_DATA_DIR>/models`). Vectors from two models are not comparable, so an index built with one is dropped, with a notice, when the server starts embedding with another: rebuild after changing it. See [`semantic-search.md`](./semantic-search.md#choosing-a-local-model). |
