...
| `ZOTEUS_INDEX_OWN_WORDS` | `true` | Index your own child notes and PDF annotations as passages of the item they hang off, so semantic search finds what you wrote about a paper alongside the paper itself. A hit is marked `source:"note"` or `source:"annotation"`. On by default, unlike full text: the whole census is one paged read of the library's notes and annotations plus one lookup per fifty annotated attachments, with no per-item request. See [`semantic-search.md`](./semantic-search.md#your-own-notes-and-annotations). |
