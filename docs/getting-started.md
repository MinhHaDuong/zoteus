...
| `Full-text indexing is refused here ... running on Electron` when you ask for a full-text index build | Expected, and deliberate. The Claude app runs Zoteus inside its own process (an Electron `UtilityProcess`), and a full-text build kills that process partway through with no error at all ([#37](https://github.com/oscardvs/zoteus/issues/37)); the identical build finishes under ordinary Node. Until the cause is understood Zoteus refuses the pass instead of dying in it, and leaves your existing index untouched. Build the index once from a terminal, then let the Claude app read it, and keep it current from in there with `zotero_index action:"update"`, which is never affected. Full recipe in [semantic-search.md](./semantic-search.md#full-text-builds-are-refused-inside-claude-desktop). Metadata-only builds (`fulltext:false`) work normally. |
