...
| `Server transport closed unexpectedly ... process exiting early` in the Claude Desktop log | Zoteus exits when the host closes its input stream, which is a normal shutdown and not a crash. Since 1.7.2 it says so on stderr before it goes: look for `The host closed the stdio connection ...` in the same log. If that line is there, something on the host side ended the session; if it is not, the process died another way, so please [open an issue](https://github.com/oscardvs/zoteus/issues) with the surrounding lines. |
