...
| `Server transport closed unexpectedly ... process exiting early` in the Claude Desktop log | Zoteus exits when the host closes its input stream, which is a normal shutdown and not a crash. Since 1.7.2 it says so on stderr before it goes: look for `The host closed the stdio connection ...`. Read it in `main.log`, not in `mcp-server-Zoteus*.log` (see the row below). If that line is there, something on the host side ended the session; if it is not, the process died another way, so please [open an issue](https://github.com/oscardvs/zoteus/issues) with the surrounding lines. |
| Nothing from Zoteus in `mcp-server-Zoteus — Zotero MCP.log` | Recent Claude Desktop versions run bundled extensions in an Electron **UtilityProcess** rather than a subprocess, and that log only carries what the host itself says about the server. Zoteus's own stderr, every `[zoteus] ...` line, is written to `main.log` in the same folder, prefixed `[UtilityProcess stderr]`. A log directory with no `[zoteus]` line in it is normal; a *crash* is only visible in `main.log`. Both files live in `~/Library/Logs/Claude` (macOS) or `%APPDATA%\Claude\logs` (Windows). |
| `[zoteus] FATAL ZodError ... Expected number, received nan` on 1.7.2 and earlier | A numeric setting left empty in the extension's settings pane was not substituted by the host at all: Zoteus received the literal text `${user_config.embed_batch_size}`, read it as `NaN`, and stopped before it could log anything else. All four numeric fields are empty on a fresh install, so this affects 1.7.0 through 1.7.2 on Claude Desktop 1.37937. Fixed in 1.7.3: update the extension. On an older build, typing a number into each of the four fields (32, 0, 40000, 5000) works around it. |
