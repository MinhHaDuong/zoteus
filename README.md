...
- **Find what *you* wrote about it.** Your child notes and PDF annotations are indexed as passages of the item they hang off, so *"where did I object to the identification strategy?"* finds the paper through your own marginal comment. A hit is marked `source:"note"` or `source:"annotation"`, so you can tell your words from the author's.
