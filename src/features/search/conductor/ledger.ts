 ...
 *
 * **2 was allocated twice.** Tranche 3 (ticket 0553) and the v2 content schema (0556) were
 * built in parallel off tranche 2 and each bumped 1 → 2 for its own additions: the extract
 * shim's bookkeeping (`extract_state`, `attachment_choice`, both the conductor's, per the
 * write line §5.2.4 draws through the shim) on one side, §5.2.2's content half (`slabs`,
 * `entries`, `passages`, `fts`) on the other. Neither is wrong and neither ever left its
 * fork branch, but a stamp that means two different shapes is exactly the thing the stamp
 * exists to prevent — so **3 is their union** (ticket 0568), and a file stamped 2 by either
 * branch is a file this build cannot tell apart from the other's.
 *
 * That costs nothing, because both steps are additive: a file at 1 or at 2 is brought
 * forward by the `CREATE TABLE IF NOT EXISTS` block itself and re-stamped, whichever half
 * it was missing. And it buys the refusal in `open`: a build that understands only one half
 * declines a file stamped 3 rather than reading it as its own.
 *
 * There is one counter, not two per half: SPEC.md §5.2.2 is "one file, one schema", and a
 * file whose halves could disagree about their version is a file no reader can reason
 * about. One thing does not come forward: a version-1 file was written before `auto_vacuum`
 * was set, and that cannot be repaired in place — see `autoVacuumIncremental`.
 */
