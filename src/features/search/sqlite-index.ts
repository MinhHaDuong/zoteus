...
import { highDfMinimum, pruneTerms, MIN_MATCH_TERMS, REDERIVE_DRIFT, type HighDf } from './droplist.js';
...
export class SqliteSearchIndex extends SearchIndexBase {
  ...
  protected keywordSearch(q: string, topK: number): RankedId[] {
    const terms = pruneTerms([...new Set(tokenize(q))], this.highDf(), MIN_MATCH_TERMS);
  ...
  protected highDf(): HighDf | undefined {
   ...
   * it: measured at ~3,4 s warm and ~7,5 s cold over 639 888 terms on a 477 512-passage
   * library. That is far too slow for query time, and too slow to pay at every server
   * start. Everything else about the answer is small — 23 terms, 53 bytes at that size —
   * so the scan happens where a full walk of the corpus is already being paid for.
   *
   * **The cadence, therefore.** At the end of a full build, which already costs ~264 s on
   * that library, so this is 1,3% on top and invisible. Not on every delta, where 3,4 s
