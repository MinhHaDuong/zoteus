...
export abstract class SearchIndexBase implements SearchIndex {
  ...
  async query(q: string, opts: QueryOptions = {}): Promise<SearchHit[]> {
    ...
    // Semantic-only mode must not query stored vectors without the exact validated
    // embedder that produced them; auto/keyword retain their graceful BM25 fallback.
