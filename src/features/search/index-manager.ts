...
export function makeSnippet(text: string, query: string, max = 240): string {
  ...
  //
  // The stripped form is compared too, because the index holds it beside the written one:
  // a passage spelling `théorie` answers a query for `theorie`, and the snippet must be able
  // to find what the search found.
  ...
  for (const m of clean.matchAll(/[\p{L}\p{N}]+/gu)) {
    ...
    if (wanted.has(token) || wanted.has(foldMarks(token))) {
      pos = m.index;
      break;
    }
  }
